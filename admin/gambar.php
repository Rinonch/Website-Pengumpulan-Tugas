...
<head>
    <meta charset="UTF-8">
    <title>Gambar - Dashboard Admin</title>
...
<nav class="navbar navbar-expand-lg" style="background:#1e3a8a;">
    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-4">
            <li class="nav-item">
                <a class="nav-link active" href="gambar.php">Gambar</a>
            </li>
            <li class="nav-item">
    ...
    <div class="card">
        <div class="card-body">
            ...
            <table class="table table-bordered table-hover">
                ...
                <?php
                $no = 1;
$q = mysqli_query($conn, "SELECT * FROM tugas WHERE (file LIKE '%.jpg' OR file LIKE '%.jpeg' OR file LIKE '%.png')");
while($row = mysqli_fetch_assoc($q)) {
    $file_url = '../uploads/' . $row['file'];
    echo "<tr>
        <td>{$no}</td>
        <td>{$row['nama_siswa']}</td>
        <td>{$row['judul_tugas']}</td>
        <td><img src='{$file_url}' alt='Preview' class='img-preview'></td>
        <td><a href='{$file_url}' target='_blank'>Lihat</a></td>
